...
class Uploader:
    ...
    def initWidgets(self):
        ...
        self.strTheme = StringVar()
        lines = []
        
        #设置字体大小
        #self.labelComboboxFont = 10
        #self.labelFileFont = 13
        #self.labelTitleFont = 13
        #self.labelEntryFont = 13
        ...
        self.lastSetting = [model,strDefaultPath,strSwVersion,strBdVersion,mode,theme]
        
        #self.win.set_theme(self.lastSetting[5])
        ...
        # 设置 Button 字体
        #style = ttk.Style()
        #style.configure('my.TButton', font=('Arial', 12), foreground='blue', width = 1)
        #style.configure('my.TEntry', font=('Arial', self.labelEntryFont), foreground='green')
        ...
        self.labFileDir = ttk.Label(fmFileDir, text=txt('labFileDir'))
        self.labFileDir.grid(row=0, column=0, ipadx=2, padx=2, sticky=W)

        self.btnFileDir = ttk.Button(fmFileDir, text=txt('btnFileDir'), command=self.open_dir)  # 绑定 open_dir 方法
        self.btnFileDir.grid(row=0, column=1, ipadx=1, padx=5, pady=5, sticky=E)

        self.entry = ttk.Entry(fmFileDir, textvariable=self.strFileDir)
        ...
        self.labSoftwareVersion = ttk.Label(fmSoftwareVersion, text=txt('labSoftwareVersion'))
        self.labSoftwareVersion.grid(row=0, ipadx=5, padx=5, sticky=W)
        self.cbSoftwareVersion = ttk.Combobox(fmSoftwareVersion, textvariable=self.strSoftwareVersion, width=regularW)
...
#        fmSoftwareVersion.rowconfigure(1, weight=1)  # 尺寸适配
        ...
        self.labBoardVersion = ttk.Label(fmBoardVersion, text=txt('labBoardVersion'))
        self.labBoardVersion.grid(row=0, ipadx=5, padx=5, sticky=W)
        
        self.cbBoardVersion = ttk.Combobox(fmBoardVersion, textvariable=self.strBoardVersion, width=regularW)
...
#        fmBoardVersion.rowconfigure(1, weight=1)  # 尺寸适配
        ...
        self.labProduct = ttk.Label(fmProduct, text=txt('labProduct'))
        self.labProduct.grid(row=0, column=0, ipadx=5, padx=5, sticky=W)
        
        cbProduct = ttk.Combobox(fmProduct, textvariable=self.strProduct, width=regularW)
        # list of product
        ...
        self.labMode = ttk.Label(fmMode, text=txt('labMode'))
        ...
        self.cbMode = ttk.Combobox(fmMode, textvariable=self.strMode, width=regularW)
        # 为 Combobox 设置默认项
        ...
        self.labPort = ttk.Label(fmSerial, text=txt('labPort'))
        self.labPort.grid(row=0,column = 0,  ipadx=5, padx=5, sticky=W)
        cb = ttk.Combobox(fmSerial, textvariable=self.strPort, width=16)
...
#        fmSerial.rowconfigure(1, weight=1)  # 尺寸适配
        ...
        self.btnUpload = ttk.Button(fmSerial, text=txt('btnUpload'), width=10, command=self.autoupload)    # 绑定 autoupload 方法 
        self.btnUpload.grid(row=0, column=1, rowspan=2, ipadx=3, padx=10, pady=1, sticky=S)

        fmStatus = ttk.Frame(self.win)
        fmStatus.grid(row=4, columnspan=2, ipadx=2, padx=2, pady=5, sticky=W + E + N + S)
        self.statusBar = ttk.Label(fmStatus, textvariable=self.strStatus, relief=FLAT)
...
#        print("listDir:" + str(listDir))
    ...
    def saveConfigToFile(self,filename):
        config = [self.defaultLan,self.lastSetting[0],self.lastSetting[1],self.lastSetting[2],self.lastSetting[3],self.lastSetting[4],self.lastSetting[5]]
        print(config)
...
#        print('exit fun')
    ...
    def autoupload(self):
        ...
        strTheme = self.strTheme.get()
        self.currentSetting = [strProd, strDefaultPath, strSoftwareVersion, strBoardVersion, strMode, strTheme]
        ...
        if strBoardVersion in NyBoard_version_list:
            ...
            for s in range(len(uploadStage)):
                if s == 0 and self.bParaUploaded and self.currentSetting[:5] == self.lastSetting[:5]:
                    continue
