...
class UI:
    ...
    def createMenu(self):
        ...
        theme = Menu(self.menubar, tearoff=0)
        for t in themes:
            theme.add_radiobutton(label=t, command=lambda themeChoice=t: self.setTheme(themeChoice))

        self.menubar.add_cascade(label=txt('labTheme'), menu=theme)
    ...
    def setTheme(self, t):
        if t == 'Dark':
            self.theme = 'darkly'
            messagebox.showinfo('Info', 'Changed theme to \"Dark\"')
        elif t == 'Light':
            self.theme = 'yeti'
            messagebox.showinfo('Info', 'Changed theme to \"Light\"')
    ...
    def utility(self, app):
        configuration = [self.defaultLan, model, self.defaultPath, self.defaultSwVer, self.defaultBdVer,
                         self.defaultMode, self.theme]
        self.saveConfigToFile('./defaultConfig.txt', configuration)
        self.window.destroy()

        if app == 'Firmware Uploader':
            Uploader(model, language, self.theme)
        elif app == 'Joint Calibrator':
            Calibrator(model, language, self.theme)
        elif app == 'Skill Composer':
            SkillComposer(model, language, self.theme)
